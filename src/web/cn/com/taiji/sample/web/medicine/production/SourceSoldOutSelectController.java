...
@Controller
@RequestMapping("/tablet/saleOut/taskSelect")
public class SourceSoldOutSelectController extends BaseLogController
{
	@Autowired
	private SourceSoldOutManager soldOutManager;
	@Autowired
	private SourceProductionManager productionManager;
	private final String prefix = "tablet/saleOut/taskSelect/";

	@RequestMapping(value = "/manage", method = RequestMethod.GET)
	public String manageGet(@ModelAttribute("queryModel") SourceProductionPageRequest req)
	{
	...
	{
		model.addAttribute("pagn", productionManager.queryPage(req));
		System.out.println("select --432-");
		return prefix + "queryResult";
	}
	@RequestMapping(value = "/createTask/{id}", method = RequestMethod.GET)
	public String createTask(@PathVariable("id") String id,@ModelAttribute("pageModel") SourceSoldOut req, Model model)
	{
...
//		System.out.println("findById:"+findById);
		model.addAttribute("task", productionManager.findById(id)); 
		return prefix + "createTask";
	}
	@RequestMapping(value = "/createTask", method = RequestMethod.POST)
	public String createTaskPost(@ModelAttribute("pageModel") SourceSoldOut req, HttpServletResponse response, Model model,
			HttpServletRequest request)
	{
		soldOutManager.createTask(req,request);
