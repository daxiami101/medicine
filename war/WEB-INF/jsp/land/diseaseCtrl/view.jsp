	 	...
	 	<div class="modal-body">
		  	<table class="table table-bordered">
		  	 <tr style="background:#f0f3f5">
				...
				<tr>
					<th>主要病害种类 </th>
					<td>${fn:escapeXml(model.diseaseKind) }</td>
					<th>危害部位</th>
					<td>${fn:escapeXml(model.diseasePalce) }</td>
				</tr>
				<tr>
					<th>农药种类</th>
					<td>${fn:escapeXml(model.drugKind) }</td>
					<th>防治开始时间</th>
					<td><fmt:formatDate value="${model.preventStartTime.time}" pattern="yyyy-MM-dd"/></td>
				</tr>
				<tr>
					<th>防治结束时间</th>
					<td><fmt:formatDate value="${model.preventEndTime.time}" pattern="yyyy-MM-dd"/></td>
					<th>防治方式 </th>
