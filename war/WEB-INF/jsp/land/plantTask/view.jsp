	 	...
	 	<div class="modal-body">
		  	<table class="table table-bordered">
		  	 <tr style="background:#f0f3f5">
				...
				<tr>
					<th>种植方式</th>
					<td>${fn:escapeXml(model.plantMethod.value) }</td>
					<th>种植起始时间</th>
					<td><fmt:formatDate value="${model.startPlantTime.time}" pattern="yyyy-MM-dd"/></td>
				</tr>
				<tr>
					<th>种植结束时间</th>
					<td><fmt:formatDate value="${model.endPlantTime.time}" pattern="yyyy-MM-dd"/></td>
					<th>种植面积(亩)</th>
					<td>${fn:escapeXml(model.area) }</td>
				</tr>
				<tr>
					<th>产地</th>
					<td>${fn:escapeXml(model.originalPalce) }</td>
					<th></th>
					<td></td>
				</tr>
				<tr style="background:#f0f3f5">
