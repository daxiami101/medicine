	 	...
	 	<div class="modal-body">
		  	...
		  	<tr style="background:#f0f3f5">
		  	 ...
		  	 <tr style="background:#f0f3f5">
				...
				<tr>
					<th>包装时间 </th>
					<td><fmt:formatDate value="${model.packageDate.time}" pattern="yyyy-MM-dd"/></td>
					<th>包装规格</th>
					<td>${fn:escapeXml(model.standard) }</td>
				</tr>
				<tr>
					<th>包装重量</th>
					<td>${fn:escapeXml(model.weight) }</td>
					<th>包装材料</th>
					<td>${fn:escapeXml(model.packageMaterial) }</td>
				</tr>
				<tr>
					<th>品名</th>
					<td>${fn:escapeXml(model.name) }</td>
					<th>等级</th>
					<td>${fn:escapeXml(model.level) }</td>
				</tr>
				<tr>
					<th>产地</th>
