		...
		<div class="modal-header">
	 	...
	 	<div class="modal-body">
		  	...
		  	<tr style="background:#f0f3f5">
		  			<th colspan='4' style="text-align:center">采购信息</th>
		  		</tr>
		  		<tr>
					<th>原料名称</th>
					<td>${fn:escapeXml(purchaseModel.materialKind) }</td>
					<th>运输方式</th>
					<td>${fn:escapeXml(purchaseModel.materialName) }</td>
				</tr>
		  		<tr>
					<th>药材编码</th>
					<td>${fn:escapeXml(purchaseModel.medicineCode) }</td>
					<th>产地</th>
					<td>${fn:escapeXml(purchaseModel.province) }</td>
				</tr>
		  		<tr>
					<th>等级</th>
					<td>${fn:escapeXml(purchaseModel.level) }</td>
					<th>规格</th>
					<td>${fn:escapeXml(purchaseModel.standard) }</td>
				</tr>
		  		<tr>
					<th>到货单据号</th>
					<td>${fn:escapeXml(purchaseModel.orderId) }</td>
					<th>随货同行单据号</th>
					<td>${fn:escapeXml(purchaseModel.companyOrderId) }</td>
				</tr>
		  		<tr>
					<th>计量单位</th>
					<td>${fn:escapeXml(purchaseModel.unit) }</td>
					<th>采购数量(个)</th>
					<td>${fn:escapeXml(purchaseModel.purchaseNum) }</td>
				</tr>
		  		<tr>
					<th>采收日期</th>
					<td><fmt:formatDate value="${purchaseModel.purchaseTime.time}" pattern="yyyy-MM-dd"/></td>
					<th>采购数量(个)</th>
					<td>${fn:escapeXml(purchaseModel.purchaseNum) }</td>
				</tr>
		  		<tr>
					<th>供货商</th>
					<td>${fn:escapeXml(purchaseModel.providerName) }</td>
					<th>采购批号</th>
					<td>${fn:escapeXml(purchaseModel.purchaseNo) }</td>
				</tr>
		  	<tr style="background:#f0f3f5">
		  		...
		  		<tr>
					<th>药材名称</th>
					<td>${fn:escapeXml(transportModel.medicineName) }</td>
					<th>运输方式</th>
					<td>${fn:escapeXml(transportModel.transMethod) }</td>
				</tr>
		  		<tr>
					<th>安保措施</th>
					<td>${fn:escapeXml(transportModel.securityMeasure) }</td>
					<th>安保合同号</th>
					<td>${fn:escapeXml(transportModel.securityContractCode) }</td>
				</tr>
		  		<tr>
					<th>饮片编码</th>
					<td>${fn:escapeXml(transportModel.medicineCode) }</td>
					<th>随货单据号</th>
					<td>${fn:escapeXml(transportModel.companyOrderNo) }</td>
				</tr>
		  		<tr>
					<th>销售时间</th>
					<td><fmt:formatDate value="${transportModel.purchaseDate.time}" pattern="yyyy-MM-dd"/></td>
					<th>运输时间</th>
					<td><fmt:formatDate value="${transportModel.transportTime.time}" pattern="yyyy-MM-dd"/></td>
				</tr>
		  	<tr style="background:#f0f3f5">
...
<!-- 				<tr> -->
<!-- 					<th>产地</th> -->
<%-- 					<td>${fn:escapeXml(model.place) }</td> --%>
<!-- 					<th>采收日期</th> -->
<%-- 					<td><fmt:formatDate value="${model.harvestDate.time}" pattern="yyyy-MM-dd"/></td> --%>
<!-- 				</tr> -->
<!-- 				<tr> -->
<!-- 					<th>包装批次号</th> -->
<%-- 					<td>${fn:escapeXml(model.packageNo) }</td> --%>
<!-- 					<th>生产单位</th> -->
