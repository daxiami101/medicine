 ...
 * @requires    base64
 * 
 * Copyright  &copy; 2010-2018 TaiJi Computer Corporation Limited
 * @author: Tom (tomesc@msn.com)
 * 
 * Examples and documentation at: http://js.taiji.com.cn/
 * 
 * @version 3.7.2(2018-09-12)
 * 
 * @since 1.0.0
 * 
 *****************************************************************************/
...
//			 主动调用一次查询
		...
		eventHandler:{
			...
			_handleSearchSuccess : function(responseText) {
				...
				//以前拼错一个单词，优先使用错的单词
				var searchResultType=this.settings.search.seachResultType||this.settings.search.searchResultType;
				if(searchResultType==='tbody'){
				...
				}else if(searchResultType==='table'){
			...
			_resultTableFloat : function(){
				...
				}else if(myTableHeader.size()>0&&searchResultType==='tbody'){
					myTableHeader.find(">table>tbody").replaceWith(searchResult.find("table>tbody").clone());
				}else if(myTableHeader.size()>0&&searchResultType==='table'){//动态表格时每次都复制table
		...
		config:{
			...
			searchResultType : 'tbody',				//查询结果类型(tbody,table,other)
