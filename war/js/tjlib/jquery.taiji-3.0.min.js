...
var searchResultType=this.settings.search.seachResultType||this.settings.search.searchResultType;if(searchResultType==="tbody"){var hasText=$responseText.find("#taiji_search_data tbody").children().size()>0;if(hasText===true){var $tbody=$responseText.find("#taiji_search_data tbody");this.aclCheck($tbody);$resultTarget.find("> table > tbody").empty().append($tbody.html());}else{$resultTarget.find("> table > tbody").empty().append("<tr><td colspan='15' ><div class='taiji_not_found'>没有检索到符合条件的数据！</div></td></tr>");}this._bindDataClick();this._resultTableFloat();}else{if(searchResultType==="table"){this.aclCheck($searchData);$resultTarget.empty().append($searchData.html());this._bindDataClick();this._resultTableFloat();}else{this.aclCheck($searchData);$resultTarget.empty().append($searchData.html());}}var $this=this;if(this.searchAutoRefreshEnable===true){if(this.taijiSearchTimeoutId){window.clearTimeout(this.taijiSearchTimeoutId);}this.taijiSearchTimeoutId=window.setTimeout(function(){$($this.searchSubmitClassName,$this.currentTarget).trigger("click");
